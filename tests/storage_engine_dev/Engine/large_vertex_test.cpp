...
const value_type kBaseKeyValue = "bibobuba";
...
TEST_F(DatabaseTest, LargeVertexInsert) {
  storage_engine_type engine;
  const auto vertex = get_large_key('0');
  engine.add_vertex(kBaseKeyValue, vertex);
  auto finded_vertex = engine.get_vertex(kBaseKeyValue);
  ASSERT_TRUE(finded_vertex.has_value());
  ASSERT_EQ(vertex_getter::get(finded_vertex.value(), 0), vertex);
}
