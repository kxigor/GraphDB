...
#include "utils/storage_files.hpp"
...
class KeyManager {
  ...
  using id_pos_type = files::FileArrayPos;
 ...
 private:
  ...
  id_pos_type calculate_id_pos(const id_type& id) {
    return files::calculate_file_pos(id, kDefaultKeyArrayCapacity);
  }
...
class VertexManager {
  /*============================= Usings ==============================*/
  using vertex_pos = files::FileArrayPos;
 ...
 private:
  ...
  static vertex_pos calculate_vertex_pos(const id_type& id) noexcept {
    return files::calculate_file_pos(id, kDefaultVertexArrayCapacity);
  }
...
class EdgeManager {
 ...
 public:
  ...
  void add_edge(const id_type& id_from, const id_type& id_to,
    ...
    forward_edges.add_edge(id_to, kReverseEdgesLastPos, std::move(edge_fields));
