...
class StorageEngine {
 ...
 private:
  ...
  ids_vertexes_type get_neighbours(const id_type& id, const depth_type& depth) {
    ...
    unordered_set_type<id_type> visited_vertexes{id};
    queque_type<id_type> vertexes_to_visit;

    auto append_unvisited_to_queue = [&](const id_type& source_id) {
      for (auto adding_id : edge_manager_.get_forward_ids(source_id)) {
        if (visited_vertexes.contains(adding_id)) {
          continue;
        }
        vertexes_to_visit.push(adding_id);
      }
    };

    append_unvisited_to_queue(id);
    for (size_type cur_depth = 0;
         cur_depth < depth && !vertexes_to_visit.empty(); ++cur_depth) {
      const size_type kLevelSize = vertexes_to_visit.size();
      for (size_type cur_ctr = 0; cur_ctr < kLevelSize; ++cur_ctr) {
        id_type cur_id = vertexes_to_visit.front();
        vertexes_to_visit.pop();
        visited_vertexes.insert(cur_id);
        auto vertex = vertex_manager_.get_vertex(cur_id);
        neighbours.emplace_back(cur_id, std::move(vertex));
        append_unvisited_to_queue(cur_id);
      }
    }
    return neighbours;
  }
